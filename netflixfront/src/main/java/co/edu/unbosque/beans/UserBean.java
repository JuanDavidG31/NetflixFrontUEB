...
/**
 * Bean que gestiona las interacciones del usuario, incluyendo el registro, inicio de sesión y
 * recuperación de contraseña. Esta clase utiliza DAOs (Objetos de Acceso a Datos) para interactuar
 * con los datos del usuario y de Excel. Es un bean de alcance de sesión, lo que significa que su
 * estado se mantiene a través de múltiples solicitudes dentro de una sesión de usuario.
 */
@Named("UserBean")
@SessionScoped
public class UserBean implements Serializable {
  ...
  /**
   * Inicia sesión del usuario, registra la hora de inicio de sesión y redirige a la página del
   * menú.
   */
  public void iniciarSesion() {
    ...
    for (UserDTO u : userList) {
      ...
      if ((tusuario.equals(email) || temail.equals(email)) && tpassword.equals(password)) {
        ...
        eDao.exportToCSV("Usuario.csv", nuevoUsuario);
  ...
  /**
   * Inicia el proceso de recuperación de contraseña redirigiendo a la página de nueva contraseña si
   * se encuentra el correo electrónico/nombre de usuario. Muestra un mensaje de error si el correo
   * electrónico/nombre de usuario es incorrecto.
   */
  public void recuperar() {
    ...
    FacesContext.getCurrentInstance()
        .addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "ID incorrecto."));
  }

  /**
   * Cambia la contraseña del usuario si las nuevas contraseñas coinciden y redirige a la página de
   * inicio.
   */
